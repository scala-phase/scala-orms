...
object BorrowerDAO extends SalatDAO[Borrower, ObjectId](collection = db.borrower) {

  class BorrowalCollection(collection: MongoCollection, parentIdField: String)
    extends ChildCollection[Borrowal, ObjectId](collection, parentIdField)

  val borrowal = new BorrowalCollection(collection = db.borrowal, parentIdField = "borrowerId")
